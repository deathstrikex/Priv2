...
#Datasets
source("table_data.R")
# load("Table_try v3 20251110 v2 workspace.RData")
...
t1<-GE1.v2 %>% select(ENTITY_NAME,sum_BANK_MARKET_VALUE2) %>% 
  ...
  mutate(ENTITY_NAME=gsub("Global Equity","",ENTITY_NAME)) %>%
...
t2 <- FI1.v2 %>% select(ENTITY_NAME,PctofFIxTrans) %>% 
  ...
  mutate(ENTITY_NAME=gsub("Fixed Income","",ENTITY_NAME)) %>% 
...
t3<-AC.v2 %>% select(ENTITY_NAME,PctofFIxTrans) %>% 
  ...
  rename(ActualAllocation = PctofFIxTrans)%>%
  mutate(ENTITY_NAME=paste0(" ",ENTITY_NAME)) %>%
  bind_rows(tibble(ENTITY_NAME = "Active Credit"))%>% 
  slice(n(), 1:(n()-1))  
  

t4 <- data.frame(ENTITY_NAME = c(" Core"," Non-Core"),
...
t_all2 <- t_all %>%
  mutate(
    raw_label = ` `,
    lead_spaces = nchar(raw_label) - nchar(str_trim(raw_label, side = "left")),
    clean_label = str_trim(raw_label, side = "left"),
    ` ` = ifelse(
      lead_spaces > 0,
      paste0(
  ...
  select(-raw_label, -lead_spaces, -clean_label)

kbl(
  select(t_all2, -Table),
  row.names = FALSE,
  escape = FALSE   # IMPORTANT: allow HTML
) %>%
  add_header_above(
    setNames(ncol(t_all2) - 1, title_text),
    bold = TRUE,
    color = "white",
    background = "#233D7B",
...
t8 <- data.frame(ENTITY_NAME = c("Fixed Income ex Transition"," Active","    Core","    Core Plus"," Passive"),
...
kbl(
  ...
  add_header_above(
    setNames(ncol(t_all2) - 1, header_text),
    bold = TRUE,
    color = "white",
    background = "#233D7B",
    extra_css = "font-size:18px; text-align:center; vertical-align:middle;"
